...
export default function Work() {
  ...
  const handleAddProject = () => {
    let nextProjectId = getUniqueProjectId()
    let nextProjectName = getUniqueProjectName()
    const project = {
      id: nextProjectId,
  ...
  const getUniqueProjectName = (name: string = 'Project_1') => {
    let nextProjectName = name
    projects.forEach((p) => {
      if (p.name === nextProjectName) {
        let nameArray = nextProjectName.split('_')
  ...
  const handleEditProjectName = (id: number, name: string) => {
    let nextProjectName = getUniqueProjectName(name)
  ...
  const handleAddDiagram = (projectId: number) => {
    let nextDiagramId = getUniqueDiagramId(projectId)
    let nextDiagramName = getUniqueDiagramName(projectId)
  ...
  const getUniqueDiagramName = (projectId: number, name: string = 'Diagram_1') => {
    let nextDiagramName = name
    let targetProject = projects.find((p) => p.id === projectId)
    targetProject?.diagrams.forEach((d) => {
      if (d.name === nextDiagramName) {
        let nameArray = nextDiagramName.split('_')
        if (nameArray.length === 1 || Number.isNaN(Number(nameArray[nameArray.length - 1]))) {
          nameArray.push('1')
        } else {
          nameArray[nameArray.length - 1] = (Number(nameArray[nameArray.length - 1]) + 1).toString()
  ...
  const handleEditDiagramName = (projectId: number, diagramId: number, name: string) => {
    let nextDiagramName = getUniqueDiagramName(projectId, name)
    if (!nextDiagramName) {
      console.log('Invalid projectId')
      return
    }
